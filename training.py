...
import os
import random
train_data_dir = "C:\\Users\\Petr\\PycharmProjects\\sc2AI\\train_data"
...
if __name__ == '__main__':
    ...
    model.compile(loss='categorical_crossentropy',
                  optimizer=opt,
                  metrics=['accuracy'])
    tensorboard = TensorBoard(log_dir="C:\\Users\Petr\\PycharmProjects\\sc2AI\\logs\\STAGE1")
    hm_epochs = 10
    for i in range(hm_epochs):
        ...
        while not_maximum:
            ...
            test_size = 100
            batch_size = 128
            x_train = np.array([i[1] for i in train_data[:-test_size]]).reshape(-1, 184, 176, 3)
            y_train = np.array([i[0] for i in train_data[:-test_size]])
            x_test = np.array([i[1] for i in train_data[-test_size:]]).reshape(-1, 184, 176, 3)
